...
fun longestSubarray(nums: IntArray, limit: Int): Int {
    val decreasingDeque = ArrayDeque<Int>()
    val increasingDeque = ArrayDeque<Int>()

    var left = 0
    var maxSubarrayLength = 0

    for (right in nums.indices) {
        while (decreasingDeque.isNotEmpty() && nums[decreasingDeque.last()] <= nums[right]) {
            decreasingDeque.removeLast()
        }
        decreasingDeque.addLast(right)

        while (increasingDeque.isNotEmpty() && nums[increasingDeque.last()] >= nums[right]) {
            increasingDeque.removeLast()
        }
        increasingDeque.addLast(right)

        while (nums[decreasingDeque.first()] - nums[increasingDeque.first()] > limit) {
            left++
            if (decreasingDeque.first() < left) {
                decreasingDeque.removeFirst()
            }
            if (increasingDeque.first() < left) {
                increasingDeque.removeFirst()
            }
        }

        maxSubarrayLength = maxOf(maxSubarrayLength, right - left + 1)
    }
