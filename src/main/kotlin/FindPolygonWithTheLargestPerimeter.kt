 ...
 *
 * Conversely,
 * if you have k (k >= 3) positive real numbers a1, a2, a3, ..., ak where a1 <= a2 <= a3 <= ... <= ak and a1 + a2 + a3 + ... + ak-1 > ak,
 ...
 * 3 <= n <= 105
 * 1 <= nums[i] <= 109
 * @see <a href="https://leetcode.com/problems/find-polygon-with-the-largest-perimeter/">LeetCode</a>
 */
fun largestPerimeter(nums: IntArray): Long {
    nums.sort()
    var maxPerimeter: Long = -1
    var currentSum: Long = 0
    currentSum += nums[0] + nums[1]
    for (index in 2..nums.lastIndex) {
        if (nums[index] < currentSum) {
            maxPerimeter = currentSum + nums[index]
        }
