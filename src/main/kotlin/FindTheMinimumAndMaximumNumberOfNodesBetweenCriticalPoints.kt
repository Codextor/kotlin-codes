 ...
 * 1 <= Node.val <= 10^5
 * @see <a href="https://leetcode.com/problems/find-the-minimum-and-maximum-number-of-nodes-between-critical-points/">LeetCode</a>
 *
 * Example:
 * var li = ListNode(5)
 * var v = li.`val`
...
fun nodesBetweenCriticalPoints(head: ListNode?): IntArray {
    ...
    var prevCriticalIndex = -1
    var minDistance = Int.MAX_VALUE

    while (currentNode?.next != null) {
        if (isCriticalPoint(prevNode!!, currentNode, nextNode!!)) {
            if (firstCriticalIndex == -1) {
                firstCriticalIndex = currentIndex
            } else {
                minDistance = minOf(minDistance, currentIndex - prevCriticalIndex)
            }
    ...
    val maxDistance = lastCriticalIndex - firstCriticalIndex

    return intArrayOf(minDistance, maxDistance)
}

private fun isCriticalPoint(prevNode: ListNode, currentNode: ListNode, nextNode: ListNode): Boolean {
    return (currentNode.`val` > prevNode.`val` && currentNode.`val` > nextNode.`val`) ||
