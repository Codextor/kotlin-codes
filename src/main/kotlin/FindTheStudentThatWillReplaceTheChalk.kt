...
fun chalkReplacer(chalk: IntArray, k: Int): Int {
    val chalkCumulativeSum = LongArray(chalk.size) { 0 }
    var cumulativeSum: Long = 0
    chalk.forEachIndexed { studentIndex, chalkRequired ->
        cumulativeSum += chalkRequired
        chalkCumulativeSum[studentIndex] = cumulativeSum
    }
    var remainingChalk = k % cumulativeSum
    return findStudentToReplaceChalk(chalkCumulativeSum, remainingChalk, 0, chalk.lastIndex)
}

private fun findStudentToReplaceChalk(chalkPrefixSum: LongArray, remainingChalk: Long, start: Int, end: Int): Int {
    if (start >= end) {
        return end
    }
    val mid = start + (end - start) / 2
    return if (chalkPrefixSum[mid] <= remainingChalk) {
        findStudentToReplaceChalk(chalkPrefixSum, remainingChalk, mid + 1, end)
    } else {
        findStudentToReplaceChalk(chalkPrefixSum, remainingChalk, start, mid)
    }
}
